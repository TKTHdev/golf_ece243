...
/*For front page*/
#define GOLFTHISONE_HEIGHT 240
#define GOLFTHISONE_WIDTH 320
...
/*For the front page*/
short int Buffer0[240][512];

/* Global variables */
volatile uint16_t *pixel_buffer_start;
...
/*Global bool flag to check if the game is started(= Enter is pressed)*/
bool clear_screen_flag = false;
...
// Your golfthisone array (16-bit 5-6-5 RGB data)
...
//clear the Buffer1 & Buffer 2
void clear_both_buffers(){
    // Reset both buffers completely
    ...
    // First, set back buffer to Buffer1 and clear it
    *(pixel_ctrl_ptr + 1) = (int)Buffer1;
    wait_for_vsync();
    
    // Now Buffer1 is the front buffer, clear it
    pixel_buffer_start = *pixel_ctrl_ptr;
    clear_screen();
    ...
    *(pixel_ctrl_ptr + 1) = (int)Buffer2;
    ...
    // Now Buffer2 is the front buffer, clear it
    pixel_buffer_start = *pixel_ctrl_ptr;
    clear_screen();
    
    // Set back buffer to Buffer1 again to prepare for the game loop
    *(pixel_ctrl_ptr + 1) = (int)Buffer1;
    
    // Make sure pixel_buffer_start is pointing to the current front buffer
    pixel_buffer_start = *pixel_ctrl_ptr;
}

// Draw the start page (background)
void draw_startpage(void) {
    // First, draw to Buffer1
    *(pixel_ctrl_ptr + 1) = (int)Buffer1;
    wait_for_vsync();
    pixel_buffer_start = *pixel_ctrl_ptr;
    
    // Draw a simple background pattern (blue gradient)
    for (int y = 0; y < GOLFTHISONE_HEIGHT; y++) {
        for (int x = 0; x < GOLFTHISONE_WIDTH; x++) {
            // Create a blue gradient background
            short int color = 0x001F + ((y / 8) << 5); // Blue with increasing green component
            plot_pixel(x, y, color);
        }
    }
    
    // Draw "GOLF" text in the center
    int text_x = SCREEN_WIDTH / 2 - 20;
    int text_y = SCREEN_HEIGHT / 2 - 20;
    
    // Draw "G"
    draw_line(text_x, text_y, text_x + 10, text_y, 0xFFFF);
    draw_line(text_x, text_y, text_x, text_y + 20, 0xFFFF);
    draw_line(text_x, text_y + 20, text_x + 10, text_y + 20, 0xFFFF);
    draw_line(text_x + 10, text_y + 10, text_x + 10, text_y + 20, 0xFFFF);
    draw_line(text_x + 5, text_y + 10, text_x + 10, text_y + 10, 0xFFFF);
    ...
    draw_line(text_x, text_y, text_x, text_y + 20, 0xFFFF);
    draw_line(text_x, text_y + 20, text_x + 10, text_y + 20, 0xFFFF);
    draw_line(text_x + 10, text_y, text_x + 10, text_y + 20, 0xFFFF);
    ...
    draw_line(text_x, text_y, text_x, text_y + 20, 0xFFFF);
    draw_line(text_x, text_y, text_x + 10, text_y, 0xFFFF);
    draw_line(text_x, text_y + 10, text_x + 8, text_y + 10, 0xFFFF);
    
    // Draw "Press Enter to Start" text
    text_x = SCREEN_WIDTH / 2 - 50;
    text_y = SCREEN_HEIGHT / 2 + 30;
    
    // Then draw to Buffer2 to ensure both buffers have the start page
    *(pixel_ctrl_ptr + 1) = (int)Buffer2;
    wait_for_vsync();
    pixel_buffer_start = *pixel_ctrl_ptr;
    
    // Draw the same background pattern
    for (int y = 0; y < GOLFTHISONE_HEIGHT; y++) {
        for (int x = 0; x < GOLFTHISONE_WIDTH; x++) {
            // Create a blue gradient background
            short int color = 0x001F + ((y / 8) << 5); // Blue with increasing green component
    ...
    // Draw "GOLF" text in the center again
